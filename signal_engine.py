...
from coingecko_client import coingecko
...
class SignalEngine:
    ...
    def analyze_binance_symbol(self, symbol: str) -> Optional[Setup]:
        try:
            data = self.coingecko.get_all_timeframes(symbol)
    ...
    def process_all_instruments(self) -> List[Dict[str, Any]]:
        ...
        # Check OANDA auth status
        if not self.oanda.auth_valid:
            print("OANDA: Authentication INVALID - skipping Forex/Indices scan")
            print("  Check your OANDA_ACCESS_TOKEN and OANDA_ACCOUNT_ID")
        else:
            print("OANDA: Authentication valid - proceeding with scan")

        for instrument in OANDA_INSTRUMENTS:
            if not self.oanda.auth_valid:
                print("  " + instrument + ": SKIPPED (OANDA auth failed)")
                continue
